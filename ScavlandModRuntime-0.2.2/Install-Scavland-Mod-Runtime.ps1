...
function Download-AndVerify {
    ...
    if ($actual -ne $ExpectedSha256) {
        throw "$Label SHA-256 mismatch. Expected $ExpectedSha256 but received $actual."
    }
...
if (-not (Test-Path -LiteralPath $customHost -PathType Leaf)) {
    throw 'ScavlandMelonHost.dll is missing from the package.'
}
if (-not (Test-Path -LiteralPath $customMelonLoader -PathType Leaf)) {
    throw 'ScavlandMelonLoader.dll is missing from the package.'
}
if (Get-Process -Name 'Scavland' -ErrorAction SilentlyContinue) {
    throw 'Scavland is running. Close it before installing.'
}
...
Write-Host 'Scavland Mod Runtime 0.2.2' -ForegroundColor Cyan
...
try {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $officialBepInEx 'core') -PathType Container)) {
        throw 'The official BepInEx archive has an unexpected layout.'
    }
    if (-not (Test-Path -LiteralPath (Join-Path $officialMelonLoader 'net472\MelonLoader.dll') -PathType Leaf)) {
        throw 'The official MelonLoader archive has an unexpected layout.'
    }

    if (Test-Path -LiteralPath $coreDirectory -PathType Container) {
        $backupDirectory = Join-Path $bepInExDirectory ("core.before-ScavlandModRuntime-0.2.2-$timestamp")
    ...
    if (Test-Path -LiteralPath $melonLoaderDirectory -PathType Container) {
        Move-Item -LiteralPath $melonLoaderDirectory -Destination (Join-Path $gameDirectory "MelonLoader.before-ScavlandModRuntime-0.2.2-$timestamp")
    }
